...
from .forms import LogInForm,RegistrationForm, PasswordChangeForm, StaffCreationForm, StaffDetailsForm, UserDetailsForm
from .models import *
    ...
    if request.method=="GET":
        ...
        if request.user.is_authenticated:
            ...
            if request.user.is_staff:
                if request.user.staffuser.designation == 'admin':
                    return redirect('staff_members')
                elif request.user.staffuser.designation == 'writer':
...
def user_profile(request, username):
    ...
    if request.method=="POST":
        ...
        if form_type == "user_details":
            filled_form = UserDetailsForm(request.POST, initial=user_details)
            if filled_form.has_changed():
                if filled_form.is_valid():
                    for field in filled_form.changed_data:
                        if field == "first_name":
                            user.first_name = filled_form.cleaned_data['first_name']
                        elif field == "last_name":
                            user.last_name = filled_form.cleaned_data['last_name']
                        elif field == "email":
                            ...
                            if email_qs.exists():
                                ...
                                user.email = filled_form.cleaned_data['email']
                        elif field == "username":
                            ...
                            if username_qs.exists():
                                ...
                                user.username = filled_form.cleaned_data['username']
                    if filled_form.is_valid():
                        user.save()
                        context={
                            'user_detail_form':UserDetailsForm(initial={
                                'first_name':user.first_name,
                                'last_name':user.last_name,
                                'email':user.email,
                                'username':user.username,
